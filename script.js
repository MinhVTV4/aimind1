...
function makeForeignTextClickable(container) {
    // Regex for Chinese, Japanese (Hiragana/Katakana), Korean, and English words
    const combinedRegex = /([\u4E00-\u9FFF]+|[\u3040-\u309F\u30A0-\u30FF]+|[\uAC00-\uD7AF]+|[a-zA-Z']+)/g;
    ...
    nodesToProcess.forEach(textNode => {
        ...
        combinedRegex.lastIndex = 0; // Reset regex lastIndex for each node
        if (!combinedRegex.test(text)) {
            return;
        }
        combinedRegex.lastIndex = 0; // Reset again after test
        ...
        while ((match = combinedRegex.exec(text)) !== null) {
            ...
            } else if (englishRegex.test(match[0])) { // Check for English before Chinese as Chinese might contain English letters
                span.dataset.lang = 'en-US'; 
            } else { // Assume Chinese for remaining CJK characters
